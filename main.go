...
func init() {
	...
	mel.Upgrader.CheckOrigin = func(r *http.Request) bool {
		// aqui se puede hacer la validacion del dominio del navegador
		/*
			if r.Host == "midominio.com"{
				return true
			}*/
		return true
	}
...
func login(c echo.Context) error {
	...
	err := c.Bind(u)
	if err != nil {
		log.Printf("estructura usuario no valida: %v", err)
	}
	log.Print(u)
	isAuthorized := false
...
func loadUsers() []UserRequest {
	...
	urs = append(urs, UserRequest{"Alexandr", "123"}, UserRequest{"Andrea", "123"}, UserRequest{"Mariluz", "123"})
	return urs
}
...
func websocket(c echo.Context) error {
	...
	response := make(map[string]string)
	response["message"] = "no estas autorizado para ingresar al WS"
...
func mensaje(s *melody.Session, msg []byte) {
	...
	default:
		log.Printf("tipo no es porcesado")
	}
}
